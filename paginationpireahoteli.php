...
$query = "SELECT COUNT(*) as num FROM tekst WHERE kapaciteti='Пиреа/Хотели'";
...
// Get page data
$query1 = "SELECT * FROM tekst WHERE kapaciteti='Пиреа/Хотели' ORDER BY id DESC LIMIT $start, $limit";
...
if ($lastpage > 1) {
    ...
    if ($page > 1) {
        $paginate .= '<li><a href="/pireahoteli/' . $prev . '/" aria-label="Previous"><i class=" glyphicon glyphicon-chevron-left"></i></a></li>';
    } else {
    ...
    if ($lastpage < 7 + ($stages * 2)) {
        ...
        for ($counter = 1; $counter <= $lastpage; $counter++) {
            if ($counter == $page) {
                $paginate .= '<li class="active"><a href="/pireahoteli/' . $counter . '/">' . $counter . '</a></li>';
            } else {
                $paginate .= '<li><a href="/pireahoteli/' . $counter . '/">' . $counter . '</a></li>';
            }
        }
        ...
        if ($page < 1 + ($stages * 2)) {
            for ($counter = 1; $counter < 4 + ($stages * 2); $counter++) {
                if ($counter == $page) {
                    ...
                    $paginate .= '<li><a href="/sitonijahoteli/' . $counter . '/">' . $counter . '</a></li>';
                }
            }
        ...
        elseif ($lastpage - ($stages * 2) > $page && $page > ($stages * 2)) {
            ...
            $paginate .= "<li><a href='#'>...</a></li>";
            $paginate .= '<li><a href="/pireahoteli/' . $LastPagem1 . '/">' . $LastPagem1 . '</a></li>';
            pireahoteli        } // End only hide early pages
        else {
            $paginate .= "<li><a href='/1/'>1</a></li>";
            $paginate .= "<li><a href='/2/'>2</a></li>";
    ...
    // Next
    if ($page < $counter - 1) {
        $paginate .= '<li><a href="/sitonijahoteli/' . $next . '/" aria-label="&laquo;"><i class=" glyphicon glyphicon-chevron-right"></i></a></li>';
    } else {
