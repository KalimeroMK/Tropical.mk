...
$query = "SELECT COUNT(*) as num FROM tekst";
...
// Get page data
$query1 = "SELECT * FROM tekst WHERE approved ='1'ORDER BY id DESC LIMIT $start, $limit";
...
if ($lastpage > 1) {
    ...
    if ($lastpage < 7 + ($stages * 2)) {
        ...
        for ($counter = 1; $counter <= $lastpage; $counter++) {
            if ($counter == $page) {
                $paginate .= '<li class="active"><a href="/' . $counter . '">' . $counter . '</a></li>';
            } else {
    ...
    // Next
    if ($page < $counter - 1) {
        $paginate .= '<li><a href="/' . $next . '" aria-label="&laquo;"><i class=" glyphicon glyphicon-chevron-right"></i></a></li>';
    } else {
