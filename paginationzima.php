...
$query = "SELECT COUNT(*) as num FROM tekst WHERE type='Зима'";
...
// Get page data
$query1 = "SELECT * FROM tekst WHERE type='Зима' ORDER BY id DESC LIMIT $start, $limit";
...
$paginate .= '<nav class="text-center">';
    $paginate .= '<ul class="pagination">';
        ...
        if ($page > 1) {
        $paginate .= '<li><a href="/zima/' . $prev . '/" aria-label="Previous"><i class=" glyphicon glyphicon-chevron-left"></i></a></li>';
        } else {
        ...
        if ($counter == $page) {
        $paginate .= '<li class="active"><a href="/zima/' . $counter . '/">' . $counter . '</a></li>';
        } else {
        $paginate .= '<li><a href="/zima/' . $counter . '/">' . $counter . '</a></li>';
        }
        }
        ...
        $paginate .= "<li><a href='#'>...</a></li>";
        $paginate .= '<li><a href="/zima/' . $LastPagem1 . '/">' . $LastPagem1 . '</a></li>';
        $paginate .= '<li><a href="/zima/' . $lastpage . '/">' . $lastpage . '</a></li>';
        ...
        // Next
        if ($page < $counter - 1) {
        $paginate .= '<li><a href="/zima/' . $next . '/" aria-label="&laquo;"><i class=" glyphicon glyphicon-chevron-right"></i></a></li>';
        } else {
